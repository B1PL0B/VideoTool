...
export default function AudioMerger() {
  ...
  const handleMerge = async () => {
    ...
    try {
      ...
      const a=`a_${audioFile.name.replace(/[^a-zA-Z0-9.]/g,'_')}`;
      const o=`muxed_${v}`;
  ...
  const ModeCard=({id,label,sub})=>(
    <label className="flex-1 p-4 rounded-xl border cursor-pointer transition-all duration-150"
      ...
      <div className="text-sm font-heading font-semibold mb-1" style={{ color:mode===id?'#14b8a6':'var(--text-primary)' }}>{label}</div>
      <div className="text-xs font-body" style={{ color:'var(--text-muted)' }}>{sub}</div>
    </label>
  );

  const FilePill=({f,onRemove,icon:Ic,iconClass,label})=>(
    <div className="flex flex-col gap-2 p-4 rounded-2xl" style={{ background:'var(--bg-pill)', border:'1px solid var(--border-card)' }}>
      <div className={`flex items-center gap-2 mb-1 ${iconClass}`}><Ic size={15}/><span className="text-xs font-heading font-semibold uppercase tracking-wider">{label}</span></div>
      <span className="text-sm font-body truncate flex-1" style={{ color:'var(--text-primary)' }}>{f.name}</span>
      <button onClick={onRemove} className="text-xs cursor-pointer transition-colors text-left mt-1"
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-2 gap-5">
        {!videoFile ? <FileUploader onFileSelect={setVideoFile} icon="video" title="Video File" description="The base video" />
          : <FilePill f={videoFile} onRemove={()=>setVideoFile(null)} icon={Film} iconClass="text-teal-500" label="Video" />}
        {!audioFile ? <FileUploader onFileSelect={setAudioFile} accept="audio/*" icon="audio" title="Audio Track" description="Replacement soundtrack" />
          : <FilePill f={audioFile} onRemove={()=>setAudioFile(null)} icon={Music} iconClass="text-amber-500" label="Audio" />}
      </div>
      ...
      <div>
        ...
        <div className="flex gap-3">
          <ModeCard id="replace" label="Replace Audio" sub="Strips original audio completely" />
          <ModeCard id="add" label="Add as Track 2" sub="Keeps original + adds new as Track 2" />
        </div>
      </div>
